...
void expandirPeano(const char *entrada, char *saida) {
    ...
    for (int i = 0; i < len; i++) {
        if (entrada[i] == 'X') {
            strcat(saida, "XFYFX+F+YFXFY-F-XFYFX");
            j += 20;
        } else if (entrada[i] == 'Y') {
            strcat(saida, "YFXFY-F-XFYFX+F+YFXFY");
            j += 20;
        } else {
            saida[j] = entrada[i];
            j++;
...
int main() {
    ...
    FILE *arquivo = fopen("peano.txt", "w");
    FILE *entradas = fopen("entradas2.txt", "w");
    if (arquivo == NULL) {
    ...
    printf("As sequências foram salvas no arquivo 'peano.txt'.\n");
